...
exports.simulateStakeholder = functions.https.onCall(async (data, context) => {
  ...
  const { scenario, transcript, userMessage, turnCount } = data;
...
exports.getCoachingHint = functions.https.onCall(async (data, context) => {
  ...
  const { scenario, transcript, turnCount } = data;
...
Coaching hint:`;
    ...
    return {
      hint: completion.data.choices[0].message.content
    };
  } catch (error) {
  ...
  .onUpdate(async (change, context) => {
    ...
    if (after.state === 'EVALUATED' && before.state !== 'EVALUATED') {
      ...
      for (const doc of sessionsSnapshot.docs) {
        ...
        if (session.evaluation?.overall_score) {
